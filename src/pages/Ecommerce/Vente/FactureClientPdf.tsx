// src/Components/FactureClient/FacturePDF.tsx
...
const FacturePDF: React.FC<FacturePDFProps> = ({ facture, companyInfo }) => {
  // Calculer les totaux selon la même logique que FactureFournisseur
  let subTotalValue = 0;
  let totalTaxValue = 0;
  ...
  // Determine what to display as the final amount
  const displayFinalTotal = remiseValue > 0 || timbreFiscal;
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.invoiceInfo}>
            <Text style={styles.invoiceTitle}>FACTURE CLIENT</Text>
        ...
        {/* Informations du client - Simple text */}
        <View style={styles.clientSection}>
          <Text style={styles.clientTitle}>Client: {facture.client?.raison_sociale || 'Non spécifié'}</Text>
          <Text style={styles.clientText}>Adresse: {facture.client?.adresse || 'Non spécifié'}</Text>
          <Text style={styles.clientText}>Matricule Fiscal: {facture.client?.matricule_fiscal || 'Non spécifié'}</Text>
          <Text style={styles.clientText}>Téléphone: {facture.client?.telephone1 || 'Non spécifié'}</Text>
        </View>
    ...
    <View style={styles.totalsBox}>
      ...
      {remiseValue > 0 && (
        <View style={styles.totalRow}>
          <Text style={styles.summaryLabel}>
            {timbreFiscal ? "Total Final" : "Total Final"}
          </Text>
        ...
        <View style={styles.cachetSignatureSection}>
          <View style={styles.cachetContainer}>
            <View style={styles.cachetBox}>
              ...
              <Text style={styles.subText}>Du Client</Text>
            </View>
          </View>

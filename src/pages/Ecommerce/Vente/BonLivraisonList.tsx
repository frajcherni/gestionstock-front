...
import { createClient, createArticle, fetchFournisseurs, fetchCategories } from "../../../Components/Article/ArticleServices";
...
const [selectedBonLivraisonForPdf, setSelectedBonLivraisonForPdf] = useState<BonLivraison | null>(null);
...
const openPdfModal = (bonLivraison: BonLivraison) => {
  setSelectedBonLivraisonForPdf(bonLivraison);
  setPdfModal(true);
};
      ...
      if (clientSearch.length >= 1) {
          const filtered = clients.filter(
              (client) =>
                  ...
                  (client.telephone1 && client.telephone1.toLowerCase().includes(clientSearch.toLowerCase())) ||
                  (client.telephone2 && client.telephone2.toLowerCase().includes(clientSearch.toLowerCase()))
          );
    ...
    const fetchData = useCallback(async () => {
        try {
            setLoading(true);
            const [livraisonData, clientsData, vendeursData, articlesData, commandesData , categoriesData, fournisseursData] = await Promise.all([
                FetchBonLivraison(),
                fetchClients(),
                fetchVendeurs(),
                fetchArticles(),
    ...
    useEffect(() => {
      ...
      if (searchText) {
          ...
          result = result.filter(bon =>
              ...
              (bon.client?.telephone1 && bon.client.telephone1.toLowerCase().includes(searchLower)) ||
              (bon.client?.telephone2 && bon.client.telephone2.toLowerCase().includes(searchLower)) ||
...
}, [selectedArticles, showRemise, globalRemise, remiseType, isCreatingFacture, timbreFiscal, editingHT, editingTTC]);
...
const handleCreateClient = async () => {
  try {
    ...
    setNewClient({
      raison_sociale: "", designation: "", matricule_fiscal: "", register_commerce: "",
      adresse: "", ville: "", code_postal: "", telephone1: "", telephone2: "", 
      email: "", status: "Actif" as "Actif" | "Inactif"
    });
...
const calculateTTCForQuickCreate = (ht: number, tva: number, hasFodec: boolean) => {
...
const handleCreateArticle = async () => {
  try {
    ...
    const articleToCreate = {
      ...newArticle,
      pua_ttc: Number(calculateTTCForQuickCreate(newArticle.pua_ht, newArticle.tva, newArticle.taux_fodec)),
      puv_ttc: Number(calculateTTCForQuickCreate(newArticle.puv_ht, newArticle.tva, newArticle.taux_fodec)),
      categorie_id: newArticle.categorie_id ? Number(newArticle.categorie_id) : null,
      sous_categorie_id: newArticle.sous_categorie_id ? Number(newArticle.sous_categorie_id) : null,
      fournisseur_id: newArticle.fournisseur_id ? Number(newArticle.fournisseur_id) : null
    };
    ...
    setNewArticle({
      reference: "", nom: "", designation: "", puv_ht: 0, puv_ttc: 0, pua_ht: 0,
      pua_ttc: 0, qte: 0, tva: 0, remise: 0, taux_fodec: false, type: "Non Consigné" as "Consigné" | "Non Consigné",
      image: "", on_website: false, is_offre: false, is_top_seller: false,
      is_new_arrival: false, website_description: "", website_images: [], website_order: 0,
      categorie_id: "", sous_categorie_id: "", fournisseur_id: ""
    });
...
useEffect(() => {
  if (articleModal) {
    const puaTtc = calculateTTCForQuickCreate(newArticle.pua_ht, newArticle.tva, newArticle.taux_fodec);
    const puvTtc = calculateTTCForQuickCreate(newArticle.puv_ht, newArticle.tva, newArticle.taux_fodec);
...
}, [newArticle.pua_ht, newArticle.puv_ht, newArticle.tva, newArticle.taux_fodec, articleModal]);
...
useEffect(() => {
  if (newArticle.categorie_id) {
    ...
    if (!subs.find(sub => sub.id === parseInt(newArticle.sous_categorie_id))) {
...
const handleSelectBonCommande = (bon: BonCommandeClient) => {
  ...
  setSelectedArticles(bon.articles.map(item => ({
    ...
    prixTTC: item.article?.puv_ttc || ( // Use article's TTC price
    (typeof item.prixUnitaire === 'string' ? parseFloat(item.prixUnitaire) : item.prixUnitaire) * 
    (1 + ((item.tva || 0) / 100))
  ),
  ...
  setShowRemise((bon.remise || 0) > 0);
    ...
    const columns = useMemo(
      ...
      setSelectedArticles(cellProps.row.original.articles.map((item: any) => ({
          ...
          prixTTC: item.article?.puv_ttc || parseFloat(item.prix_ttc) || (parseFloat(item.prix_unitaire) * (1 + (item.tva || 0) / 100)),          tva: item.tva != null ? parseFloat(item.tva) : null,
...
<Modal isOpen={clientModal} toggle={() => setClientModal(false)} centered size="lg">
      ...
      <Col md={6}>
        <div className="mb-3">
          ...
          <Input
            ...
            onChange={(e) => setNewClient({...newClient, register_commerce: e.target.value})}
      ...
      <Col md={6}>
        <div className="mb-3">
          ...
          <Input
            ...
            onChange={(e) => setNewClient({...newClient, telephone1: e.target.value})}
      ...
      <Col md={6}>
        <div className="mb-3">
          ...
          <Input
            ...
            onChange={(e) => setNewClient({...newClient, telephone2: e.target.value})}
  ...
  <div className="modal-footer">
    <button type="button" className="btn btn-light" onClick={() => setClientModal(false)}>
      Annuler
    </button>
...
<Modal isOpen={articleModal} toggle={() => setArticleModal(false)} centered size="xl">
  <ModalHeader toggle={() => setArticleModal(false)}>
    <div className="d-flex align-items-center">
      ...
      <div>
        <h4 className="mb-0 fw-bold text-dark">Nouvel Article Rapide</h4>
        <small className="text-muted">Créer un nouvel article rapidement</small>
      </div>
    </div>
  </ModalHeader>
  <ModalBody>
    ...
    <Row>
      <Col md={6}>
        <div className="mb-3">
          <Label className="form-label">Famille Principale</Label>
          <Input
            type="select"
      ...
      <Col md={6}>
        <div className="mb-3">
          ...
          <Input
            ...
            onChange={(e) => setNewArticle({...newArticle, sous_categorie_id: e.target.value})}
          ...
          {!newArticle.categorie_id && (
            <small className="text-muted">Sélectionnez d'abord une famille principale</small>
          )}
        </div>
      </Col>
    </Row>
      ...
      <Col md={6}>
        <div className="mb-3">
          ...
          <Input
            ...
            onChange={(e) => setNewArticle({...newArticle, type: e.target.value as "Consigné" | "Non Consigné"})}
          >
    ...
    {/* Pricing Section */}
    <h6 className="fw-semibold mb-3 text-primary">Prix et Taxes</h6>
    <Row>
      <Col md={6}>
        <div className="mb-3">
          ...
          <Input
            ...
            onChange={(e) => setNewArticle({...newArticle, pua_ht: Number(e.target.value)})}
            placeholder="0.000"
          />
        </div>
      </Col>
      ...
      <Col md={6}>
        <div className="mb-3">
          ...
          <Input
            ...
            onChange={(e) => setNewArticle({...newArticle, puv_ht: Number(e.target.value)})}
            placeholder="0.000"
          />
        </div>
      </Col>
      ...
      <Col md={6}>
        <div className="mb-3">
          ...
          <Input
            ...
            onChange={(e) => setNewArticle({...newArticle, qte: Number(e.target.value)})}
            placeholder="0"
          />
        </div>
      </Col>
      <Col md={6}>
        <div className="mb-3">
          ...
          <Input
            ...
            onChange={(e) => setNewArticle({...newArticle, tva: Number(e.target.value)})}
            placeholder="0"
          />
        </div>
      </Col>
      ...
      <Col md={6}>
        <div className="mb-3">
          ...
          <Input
            ...
            onChange={(e) => setNewArticle({...newArticle, remise: Number(e.target.value)})}
            placeholder="0"
          />
        </div>
      </Col>
        ...
        <div className="mb-3 d-flex align-items-end">
          <div className="form-check form-switch">
            <Input
              ...
              onChange={(e) => setNewArticle({...newArticle, taux_fodec: e.target.checked})}
              className="form-check-input"
            />
            <Label check className="form-check-label fw-semibold">Taux FODEC (1%)</Label>
          </div>
        </div>
      </Col>
    </Row>
  ...
  <div className="modal-footer">
    <button type="button" className="btn btn-light" onClick={() => setArticleModal(false)}>
      Annuler
    </button>
    <button type="button" className="btn btn-primary" onClick={handleCreateArticle}>
...
>
    ...
    <div className="d-flex align-items-center">
      ...
      <div>
        <h4 className="mb-0 fw-bold text-dark">
          Bon de Livraison #{selectedBonLivraison?.numeroLivraison}
        </h4>
                ...
                <div className="d-flex align-items-center">
                  <div className="flex-grow-1">
                    ...
                    <p className="text-muted mb-1">
                      ...
                      {selectedBonLivraison.client?.telephone1 || "N/A"}
                    </p>
                    <p className="text-muted mb-0">
                      <i className="ri-map-pin-line me-1"></i>
                      {selectedBonLivraison.client?.adresse || "N/A"}
                    </p>
                  </div>
                </div>
              </CardBody>
            ...
            <Card className="border-0 shadow-sm h-100">
              <CardBody className="p-4">
                ...
                <div className="row g-2">
                  ...
                  <div className="col-6">
                    <p className="mb-2">
                      <span className="text-muted d-block">Commande:</span>
                      <strong>{selectedBonLivraison.bonCommandeClient?.numeroCommande || "N/A"}</strong>
                    </p>
                  </div>
                  <div className="col-6">
                    <p className="mb-2">
        ...
        {selectedBonLivraison.notes && (
          <Card className="border-0 shadow-sm mb-4">
            <CardBody className="p-4">
              ...
              <p className="mb-0 text-muted">{selectedBonLivraison.notes}</p>
            </CardBody>
          </Card>
        )}
              ...
              <Row className="justify-content-end">
                <Col xs={8} sm={6} md={5} lg={4}>
                  {(() => {
                    ...
                    selectedBonLivraison.articles.forEach((article) => {
  ...
  <ModalFooter className="border-0 pt-4">
      ...
      onClick={() => {
        if (selectedBonLivraison) {
          ...
          setSelectedArticles(selectedBonLivraison.articles.map((item: any) => ({
            ...
            quantite: item.quantite,
            prixUnitaire: parseFloat(item.prix_unitaire),
            prixTTC: parseFloat(item.prix_ttc) || (parseFloat(item.prix_unitaire) * (1 + (item.tva || 0) / 100)),
    ...
    <Button
      color="primary"
      onClick={() => selectedBonLivraison && openPdfModal(selectedBonLivraison)}
    ...
    <Row className="align-items-end">
      {isCreatingFacture ? (
        ...
        <>
          ...
          <Col md={4}>
            <div className="mb-3">
              <Label className="form-label-lg fw-semibold">Numéro de Livraison*</Label>
              <Input
          ...
          <Col md={4}>
            <div className="mb-3">
              <Label className="form-label-lg fw-semibold">Date de Livraison*</Label>
              <Input
                type="date"
                name="dateLivraison"
                value={validation.values.dateLivraison}
                onChange={validation.handleChange}
                invalid={validation.touched.dateLivraison && !!validation.errors.dateLivraison}
                className="form-control-lg"
              />
              <FormFeedback className="fs-6">
...
<div className="mb-3">
  ...
  {!isCreatingFacture && !selectedClient && clientSearch.length >= 1 && (
    <div 
          ...
          {filteredClients.map((c) => (
            ...
            >
              ...
              <div className="d-flex justify-content-between align-items-center mt-1">
                <small className="text-muted">
                  {c.designation && (
                    <span className="me-2">{c.designation}</span>
                  )}
                </small>
                {c.telephone2 && (
                  <small className="text-muted">

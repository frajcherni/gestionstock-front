...
import {
    fetchFacturesClient, createFacture, updateFacture, deleteFacture,
    fetchNextFactureNumberFromAPI, createEncaissementClient, fetchEncaissementsClient, fetchNextEncaissementNumberFromAPI
} from "./FactureClientServices";
import { fetchArticles, fetchClients , fetchVendeurs } from "../../../Components/Article/ArticleServices";
import { Article, Client, FactureClient, EncaissementClient , Vendeur } from "../../../Components/Article/Interfaces";
...
const ListFactureClient = () => {
    ...
    const [selectedFactureForPdf, setSelectedFactureForPdf] = useState<FactureClient | null>(null);
...
const companyInfo = useMemo(() => ({
    ...
    useEffect(() => {
        if (vendeurSearch.length >= 3) {
            const filtered = vendeurs.filter(vendeur =>
                `${vendeur.nom} ${vendeur.prenom}`.toLowerCase().includes(vendeurSearch.toLowerCase())
            );
            setFilteredVendeurs(filtered);
        } else {
            setFilteredVendeurs([]);
        }
    }, [vendeurSearch, vendeurs]);
    ...
    useEffect(() => {
        const fetchNextNumber = async () => {
            try {
                ...
                const defaultNumber = `FAC-C${year}${String(factures.length + 1).padStart(5, '0')}`;
    ...
    const fetchData = useCallback(async () => {
        try {
            setLoading(true);
            const [facturesData, clientsData, articlesData, encaissementsData, vendeursData] = await Promise.all([
                fetchFacturesClient(),
                fetchClients(),
                fetchArticles(),
            ...
            const facturesWithCalculatedEncaissements = facturesData.map(facture => {
                const relevantEncaissements = encaissementsData.filter(encaissement =>
                    encaissement.factureClient && encaissement.factureClient.id === facture.id
                );
            
                const totalEncaissements = relevantEncaissements.reduce((sum, encaissement) => {
                ...
                let resteAPayer = finalTotal - totalEncaissements;
                ...
                if (facture.status === "Annulee") {
                    ...
                    status = "Payee";
                } else if (totalEncaissements > 0 && totalEncaissements < finalTotal) {
    ...
    useEffect(() => {
        ...
        if (searchText) {
            ...
            result = result.filter(facture =>
                ...
                (facture.client?.raison_sociale && facture.client.raison_sociale.toLowerCase().includes(searchLower))
            );
        }
    ...
    const { subTotal, totalTax, grandTotal, finalTotal } = useMemo(() => {
        ...
        // Calculate base totals from articles (same as fournisseur calculation)
        ...
        const hasDiscount = showRemise && Number(globalRemise) > 0;
        
        // Apply discount first (same logic as fournisseur)
        if (hasDiscount) {
        ...
        // Then apply timbre fiscal (same logic as fournisseur)
        if (timbreFiscal) {
            if (hasDiscount) {
        ...
        // Fix floating point issues (same as fournisseur)
        subTotalValue = Math.round(subTotalValue * 100) / 100;
        totalTaxValue = Math.round(totalTaxValue * 100) / 100;
        grandTotalValue = Math.round(grandTotalValue * 100) / 100;
        finalTotalValue = Math.round(finalTotalValue * 100) / 100;
    ...
    const toggleCreateEditModal = useCallback(() => {
        if (createEditModal) {
            ...
            if (!isEdit) {
                fetchNextFactureNumberFromAPI().then(nextNumber => {
                    setNextFactureNumber(nextNumber);
                }).catch(err => {
                    console.error("Failed to fetch next invoice number:", err);
                    const year = moment().format('YYYY');
    ...
    const validation = useFormik({
        ...
        validationSchema: Yup.object().shape({
            ...
            conditionPaiement: Yup.string().required("La condition de paiement est requise"),
            ...
            dateEcheance: Yup.date()
                ...
                .min(Yup.ref('dateFacture'), "La date d'échéance ne peut pas être antérieure à la date de facture")
    ...
    const handleEncaissementSubmit = async (values: any) => {
        ...
        if (selectedFacture.status === "Payee" || selectedFacture.status === "Annulee") {
            toast.error("Impossible d'ajouter un encaissement pour une facture payée ou annulée.");
            return;
        }
        ...
        try {
            ...
            toast.error(err instanceof Error ? err.message : "Échec de l'enregistrement de l'encaissement");
        }
    };
 ...
 // REPLACE the StatusBadge component with this:
                ...
                cell: (cellProps: any) => {
                    ...
                    >
                        <i className="ri-file-pdf-line fs-16"></i>
                            ...
                            {facture.status !== "Annulee" && !facture.hasPayments && (                                <li className="list-inline-item edit">
                                    <Link
                                        to="#"
                                        ...
                                        onClick={() => {
                                            ...
                                            setSelectedArticles(facture.articles.map((item: FactureClient['articles'][number]) => ({
                            ...
                            <CardHeader className="card-header border-0">
                                <Row className="align-items-center gy-3">
                                    <div className="col-sm">
                                        <h5 className="card-title mb-0">Gestion des Factures Clients</h5>
                                    </div>
                                                            ...
                                                            <CardBody className="p-3">
                                                                ...
                                                                <div className="d-flex align-items-center">
                                                                    <div className="flex-grow-1">
                                                                        <h5 className="mb-1">{selectedFacture.client?.raison_sociale}</h5>
                                                                        ...
                                                                        <p className="text-muted mb-0 small">
                                                                            ...
                                                                            {selectedFacture.client?.telephone1 || 'N/A'}
                                                                        </p>
                                                                    </div>
                                                                </div>
                                                            </CardBody>
                                                                ...
                                                                <div className="row g-2">
                                                                    <div className="col-6">
                                                                        <p className="mb-1 small">
                                                                            ...
                                                                            {moment(selectedFacture.createdAt).format("DD/MM/YYYY")}
                                                                        </p>
                                                                    </div>
                                                                    <div className="col-6">
                                                                        <p className="mb-1 small">
                                                                            <span className="text-muted">Mode règlement:</span><br />
                                                                            {selectedFacture.modeReglement || 'Non spécifié'}
                                                                        </p>
                                                                    </div>
                                                                    <div className="col-6">
                                                                        <p className="mb-1 small">
                                                                            <span className="text-muted">Échéance:</span><br />
                                                                            {selectedFacture.dateEcheance ? moment(selectedFacture.dateEcheance).format("DD/MM/YYYY") : 'Non spécifiée'}
                                                                        </p>
                                                                    </div>
                                                                    <div className="col-6">
                                                                        <p className="mb-1 small">
                                                                            <span className="text-muted">Statut:</span><br />
                                                                            <StatusBadge status={selectedFacture.status} />
                                                                        </p>
                                                                    </div>
                                                                </div>
...
{selectedFacture.remise > 0 && (
                ...
                : Number(selectedFacture.remise)) + (selectedFacture.timbreFiscal ? 1 : 0)).toFixed(2)} DT
                                    ...
                                    <ModalFooter>
                                        {selectedFacture && selectedFacture.resteAPayer > 0 && selectedFacture.status !== "Annulee" && selectedFacture.status !== "Payee" && (
                                            ...
                                            >
                                                <i className="ri-money-dollar-circle-line me-1"></i> Ajouter Encaissement
                                            </Button>
                                        )}
                                        {selectedFacture && (
                                            <Button
                                                color="primary"
                                                size="md"
                                                onClick={() => openPdfModal(selectedFacture)}
                                                className="me-2"
                                            >
                                ...
                                <Modal isOpen={createEditModal} toggle={toggleCreateEditModal} centered size="lg">
                                    <ModalHeader toggle={toggleCreateEditModal}>
                                        {isEdit ? "Modifier Facture" : "Créer Facture"}
                                    </ModalHeader>
    ...
    <div className="mb-3">
        ...
        <FormFeedback>
                                                            {typeof validation.errors.dateEcheance === 'string'
                    ...
                    <div className="mb-3">
                        ...
                        <Input
                            type="text"
                            placeholder="Rechercher vendeur (min 3 caractères)"
                            value={selectedVendeur ? `${selectedVendeur.nom} ${selectedVendeur.prenom}` : vendeurSearch}
                            onChange={(e) => {
                                if (!e.target.value) {
                                    setSelectedVendeur(null);
                                    validation.setFieldValue("vendeur_id", "");
                                }
                                setVendeurSearch(e.target.value);
                            }}
                            readOnly={!!selectedVendeur}
                        />
                        {!selectedVendeur && vendeurSearch.length >= 3 && (
                            <div className="search-results mt-2">
                                        ...
                                        {filteredVendeurs.map(v => (
                                            <li
                                                ...
                                                onClick={() => {
                                                    ...
                                                    validation.setFieldValue("vendeur_id", v.id);
                                                    setVendeurSearch("");
                                                    setFilteredVendeurs([]);
                                                }}
                                            >
                        ...
                        {selectedVendeur && (
                            ...
                            >
                                <i className="ri-close-line"></i> Changer de vendeur
                            </Button>
                        )}
                   {validation.touched.vendeur_id && validation.errors.vendeur_id && (
...
<Col md={6}>
            ...
            <>
                ...
                <tr className="table-primary">
                    ...
                    <td className="text-end fw-bold">{finalTotal} DT</td>
                                ...
                                <Modal isOpen={encaissementModal} toggle={() => setEncaissementModal(false)} centered>
                                    <ModalHeader toggle={() => setEncaissementModal(false)}>
                                        Ajouter Encaissement - Facture #{selectedFacture?.numeroFacture}
                                    </ModalHeader>
                                    <Form onSubmit={encaissementValidation.handleSubmit}>
                                        <ModalBody style={{ padding: '20px' }}>
                                            <Row>
                                                <Col md={6}>
                                                    <div className="mb-3">
    ...
    invalid={encaissementValidation.touched.montant && !!encaissementValidation.errors.montant}
    placeholder="0,00"
/>
                                                        <FormFeedback>{encaissementValidation.errors.montant}</FormFeedback>
    ...
    Reste à payer: {selectedFacture?.resteAPayer?.toFixed(2).replace('.', ',')} DT
                                                ...
                                                <Col md={6}>
                                                    <div className="mb-3">
                                                        ...
                                                        <Input
                                                            ...
                                                            onBlur={encaissementValidation.handleBlur}
                                                            invalid={encaissementValidation.touched.modePaiement && !!encaissementValidation.errors.modePaiement}
                                                        >
                                                        ...
                                                        <FormFeedback>{encaissementValidation.errors.modePaiement}</FormFeedback>
                                                    </div>
                                                </Col>
                                            </Row>
                                                ...
                                                <Col md={6}>
                                                    <div className="mb-3">
                                                        ...
                                                        <Input
                                                            ...
                                                            onChange={encaissementValidation.handleChange}
                                                            onBlur={encaissementValidation.handleBlur}
                                                            invalid={encaissementValidation.touched.numeroEncaissement && !!encaissementValidation.errors.numeroEncaissement}
                                                        />
                                                        <FormFeedback>{encaissementValidation.errors.numeroEncaissement}</FormFeedback>
                                                    </div>
                                                </Col>
                                                <Col md={6}>
                                                    <div className="mb-3">
                                                        ...
                                                        <Input
                                                            ...
                                                            onChange={encaissementValidation.handleChange}
                                                            onBlur={encaissementValidation.handleBlur}
                                                            invalid={encaissementValidation.touched.date && !!encaissementValidation.errors.date}
                                                        />
                                                    </div>
                                                </Col>
                                            </Row>
                                        ...
                                        <ModalFooter>
                                            <Button color="light" onClick={() => setEncaissementModal(false)}>
                                                <i className="ri-close-line align-bottom me-1"></i> Annuler
                                            </Button>

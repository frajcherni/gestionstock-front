...
import {
    fetchBonsCommandeClient, createBonCommandeClient, updateBonCommandeClient, deleteBonCommandeClient, fetchNextCommandeNumber
...
import { createBonLivraison, fetchNextLivraisonNumber } from "../../../Components/CommandeClient/BonLivraisonServices";
...
const BonCommandeClientList = () => {
  ...
  // In your filter useEffect, update just the search part:
if (searchText) {
    ...
    result = result.filter(bon =>
      ...
      (bon.clientWebsite?.nomPrenom && bon.clientWebsite.nomPrenom.toLowerCase().includes(searchLower))
    );
  }
    ...
    const { subTotal, totalTax, grandTotal } = useMemo(() => {
        ...
        if (showRemise && globalRemise > 0) {
            if (remiseType === "percentage") {
                finalTotal = grandTotalValue * (1 - (globalRemise / 100));
            } else {
                // For fixed amount, the globalRemise IS the final amount
                finalTotal = Number(globalRemise);
            }
        }
        ...
        return {
            ...
            grandTotal: finalTotal.toFixed(3) // This should be the final amount after all discounts
        };
    ...
    const validation = useFormik({
        ...
        validationSchema: Yup.object().shape({
            dateCommande: Yup.date()
                .when('isCreatingLivraison', {
                    is: false,
                        ...
                        <Card id="bonCommandeList">
                            <CardHeader className="card-header border-0">
                                <Row className="align-items-center gy-3">
                                    <div className="col-sm">
                                        <h5 className="card-title mb-0">Gestion des Bons de Commande Client</h5>
                                    </div>
                            ...
                            <Nav className="nav-tabs nav-tabs-custom nav-success" role="tablist">
                                ...
                                <NavItem>
                                    ...
                                    >
                                        <i className="ri-truck-line me-1 align-bottom"></i> Partiellement Livré
                                    </NavLink>
                                </NavItem>
                                <NavItem>
                                    <NavLink
                                          ...
                                          // In your columns array, update just the Client column:
{
  ...
  onClick={() => {
    ...
    // Set client for display (even if it's a website client, we'll handle it in the form)
    setSelectedClient(cellProps.row.original.client || null);
    validation.setFieldValue("client_id", cellProps.row.original.client?.id || "");
                                ...
                                <Modal isOpen={detailModal} toggle={() => setDetailModal(false)} size="lg" centered>
                                    <ModalHeader toggle={() => setDetailModal(false)} className="border-0 pb-2">
                                        <div className="d-flex justify-content-between align-items-center w-100">
                                            <div>
                                                <h5 className="mb-1">Bon de Commande Client #{selectedBonCommande?.numeroCommande}</h5>
    ...
    <CardBody className="p-3">
      ...
      <div className="d-flex align-items-center">
        <div className="flex-grow-1">
          {/* Check if it's a website client */}
          {selectedBonCommande.clientWebsite ? (
            <>
              <h5 className="mb-1">{selectedBonCommande.clientWebsite.nomPrenom}</h5>
              <Badge color="info" className="mb-2">Client Site Web</Badge>
              <p className="text-muted mb-1 small">
                <i className="ri-phone-line me-1"></i>
                {selectedBonCommande.clientWebsite.telephone}
              </p>
              {selectedBonCommande.clientWebsite.email && (
                <p className="text-muted mb-1 small">
              ...
              <p className="text-muted mb-0 small">
                ...
                {selectedBonCommande.clientWebsite.adresse}
                {selectedBonCommande.clientWebsite.ville && `, ${selectedBonCommande.clientWebsite.ville}`}
                {selectedBonCommande.clientWebsite.code_postal && `, ${selectedBonCommande.clientWebsite.code_postal}`}
              </p>
            </>
          ) : selectedBonCommande.client ? (
            <>
              <h5 className="mb-1">{selectedBonCommande.client.raison_sociale}</h5>
              <Badge color="primary" className="mb-2">Client Régulier</Badge>
              <p className="text-muted mb-1 small">
                <i className="ri-phone-line me-1"></i>
                {selectedBonCommande.client.telephone1}
                {selectedBonCommande.client.telephone2 && ` / ${selectedBonCommande.client.telephone2}`}
              </p>
            ...
            <p className="text-muted mb-0">Aucun client associé</p>
          )}
        </div>
      </div>
    </CardBody>
                                                        ...
                                                        <Table className="table-borderless mb-0">
                                                                ...
                                                                <tbody>
                                                                    {selectedBonCommande.articles.map((item, index) => {
                                                                           const articleName = item.article?.designation || '';
                                                                        ...
                                                                        return (
                                                                            <tr key={index} className={index % 2 === 0 ? "bg-light" : ""}>
                                                                                <td className="ps-3">
                                                                                    <div className="d-flex align-items-center">
                                                                                        <div className="flex-grow-1">
                                                                                        <h6 className="mb-0 fs-14">{truncatedName}</h6>
                                                                                        </div>
                                                                                    </div>
                                                                                </td>
                                                                                <td>
...
{(() => {
                    ...
                    <tr>
                        ...
                        <td className="text-end">
                            - {remiseTypeValue === "percentage"
                                ...
                                : (grandTotalValue - Number(remiseValue)).toFixed(2)} DT
                        </td>
                    </tr>
                )}
                {remiseValue > 0 && (
                    <tr className="border-top">
                        ...
                        <td className="text-end fw-bold">{finalTotal.toFixed(2)} DT</td>
                                    ...
                                    <ModalFooter>
                                        ...
                                        >
                                            <i className="ri-truck-line me-1"></i> Créer Bon Livraison
                                        </Button>
                                    </ModalFooter>
                                </Modal>
                                        ...
                                        <ModalBody style={{ padding: '20px' }}>
                                            <Row>
                                                    ...
                                                    <Col md={4}>
                                                        <div className="mb-3">
                                                            ...
                                                            <Input
                                                                ...
                                                                readOnly={!isEdit} // Read-only for new livraison
                                                            />
                                                            <FormFeedback>{validation.errors.numeroLivraison}</FormFeedback>
                                                        </div>
                                                    </Col>
                                                ) : (
                                                    <Col md={4}>
                                                        <div className="mb-3">
                                                            ...
                                                            <Input
                                                                ...
                                                                readOnly={!isEdit} // Read-only for new commande
                                                            />
                                                ...
                                                <Col md={4}>
                                                    <div className="mb-3">
                                                        <Label>
                                                            {isCreatingLivraison ? "Date de livraison*" : "Date de commande*"}
                                                        </Label>
                                                        <Input
                                                            className="form-control"
                                                            type="date"
                                                            name={isCreatingLivraison ? "dateLivraison" : "dateCommande"}
                                                            value={isCreatingLivraison ? validation.values.dateLivraison : validation.values.dateCommande}
                                                            onChange={validation.handleChange}
                                                            onBlur={validation.handleBlur}
                                                            invalid={
                                                                isCreatingLivraison
                                                        ...
                                                        <FormFeedback>
                                                            {isCreatingLivraison
                                                                ? typeof validation.errors.dateLivraison === 'string'
  ...
  <Col md={6}>
    <div className="mb-3">
      <Label>Client*</Label>
      {isEdit && bonCommande?.clientWebsite ? (
        // Display website client as read-only with full details
        <div className="form-control bg-light">
          <div className="fw-medium">{bonCommande.clientWebsite.nomPrenom}</div>
          <div className="text-muted small">
            ...
            <div>
              <i className="ri-map-pin-line me-1"></i>
              {bonCommande.clientWebsite.adresse}
              {bonCommande.clientWebsite.ville && `, ${bonCommande.clientWebsite.ville}`}
              {bonCommande.clientWebsite.code_postal && `, ${bonCommande.clientWebsite.code_postal}`}
            </div>
          </div>
          <Badge color="info" className="mt-1">Client Site Web</Badge>
        </div>
      ) : (
        // Regular client selection (existing code)
        <>
          <Input
            type="text"
          ...
          />
          {!isCreatingLivraison && !selectedClient && clientSearch.length >= 3 && (
                                                        ...
                                                        <Table className="table table-bordered">
                                                            ...
                                                            <tbody>
                                                                {selectedArticles.map((item, index) => {
                                                                    ...
                                                                    const articleName = article?.designation || '';
                                                                    const truncatedName = articleName.length > 20 
                                                                      ? `${articleName.substring(0, 20)}...` 
                                                                      : articleName;
  ...
  <td title={articleName} style={{ maxWidth: '150px' }}>
          {truncatedName}
        </td>                                                                            <td>{article?.reference}</td>
                                                                            <td width="100px">
                                                                                <Input
                                                                                    type="number"
                                                                                    min="1"
                                                            ...
                                                            <tbody>
                                                                ...
                                                                <tr className="table-active">
                                                                    ...
                                                                    <td className="text-end fw-bold">
                                                                        {(Number(subTotal) + Number(totalTax)).toFixed(2)} TND
                                                                    </td>
                                                                </tr>
